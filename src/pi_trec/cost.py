...
def cost(config: CostConfig) -> None:
    ...
    for path in sorted(root.rglob("*.jsonl")):
        ...
        total_out += out
        total += tot
        rows.append({"task": path.stem, "input_tokens": inp, "output_tokens": out, "total_tokens": tot})

    input_price, output_price = _prices(config)
    cost_usd = None
    if input_price is not None and output_price is not None:
        cost_usd = total_in / 1e6 * input_price + total_out / 1e6 * output_price

    if config.output_file:
        config.output_file.parent.mkdir(parents=True, exist_ok=True)
        with config.output_file.open("w", encoding="utf-8", newline="") as handle:
            writer = csv.DictWriter(handle, fieldnames=["task", "input_tokens", "output_tokens", "total_tokens"])
            writer.writeheader()
    ...
    if cost_usd is not None:
        print(f"estimated cost: ${cost_usd:.4f} (in=${input_price}/1M, out=${output_price}/1M, model={config.model})")
    elif with_usage:
        print("estimated cost: unknown (pass --input-price and --output-price, USD per 1M tokens)")
    else:
